...
import { deleteAsyncMyComment, getAsyncUserComments } from '../../../Store/reducers/commentReducer';
...
export const MyComments = () => {
  ...
  useEffect(() => {
    dispatch(getAsyncUserComments({authToken : user["authToken"], id : user["user"]['_id']}));
  }, [change]);
                ...
                <tr key={i}>
                    ...
                    <td data-title="Delete" className='acc-td'>
                        <button className='acc-btn-delete' onClick={() =>{
                          dispatch(deleteAsyncMyComment({authToken : user["authToken"], id : comments[comment]['_id'], userId : user["user"]['_id']}));
                          setChange(change+1)}
                        }
